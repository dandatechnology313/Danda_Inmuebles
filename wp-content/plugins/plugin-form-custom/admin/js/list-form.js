...
function questionBinary(array_options = false) {
    ...
    $("#div_options").children().remove();
    var options = '<hr class="style1"><div class="control-group p-3 alert-info"><label class="label label-default font-weight-bold">Seleccione el tipo ó tipos de riesgos.</label></div><hr class="style1">';
    var selectRisk = '<div class="col-lg-4"><label class="label label-default font-weight-bold">Nivel de Riesgo</label><div class="input-group" id="selectRisk"><label class="form-check-label" for="check1"><option value="--">--</option>';
    var riksVal = '<div class="col-lg-4"></div>';
    for (let i = 1; i <= 10; i++) {
        selectRisk = selectRisk+'<option value="'+i+'">'+i+'</option><br><select><option value="0">Seleccione:</option><option value="1">Si</option><option value="0">No</option><option value="2">N/A</option></select><br>';
    }
    selectRisk = selectRisk +'</label></div></div>';
    if (array_options != false) {
        $.each(array_options, function (index, val) {
            options = options + '<div class="col-lg-6" id="label_TrueFalse"><div class="form-select p-1"><input class="form-check-input radioQuestion optTrueFalse" type="radio" name="question" id="option_' + val["id_option"] + '" value="option_' + val["id_option"] + '" ' + valid_check + '><label class="label label-default font-weight-bold" data-option=' + val["id_option"] + ' data-item="text">' + val["text_option"] + '</label></div></div>';
        });
        $("#div_options").append(options);
    }
    else {
